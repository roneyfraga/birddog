...
set.seed(888L) # for reproducibility in stochastic steps
```
...
```{r eval = run_pipeline, warning = FALSE}
...
```{r eval = run_pipeline, fig.width = 9, fig.height = 12}
...
```{r eval = requireNamespace("sessioninfo", quietly = TRUE)}
...
- Hostname: `r tryCatch(Sys.info() |> data.frame() |> janitor::clean_names() |> tibble::rownames_to_column('var') |> dplyr::filter(var == 'nodename') |> dplyr::pull(sys_info), error = function(e) "N/A")`
- Processor: `r tryCatch(benchmarkme::get_cpu()[[2]], error = function(e) "N/A")`.
- RAM: `r tryCatch(round(as.numeric(benchmarkme::get_ram()) / 1073741824, digits = 1), error = function(e) "N/A")` Gigabit.
