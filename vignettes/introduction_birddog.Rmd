...
A trajectory can stop before the final year, but with cumulative clustering its papers never leave the network: they are re-grouped each year. `sniff_trajectory_destination()` follows the papers from a trajectory's terminal node forward and reports where they land, both by destination group and by the living trajectory that absorbed them. `plot_trajectory_destination()` then draws each destination as a variable-width line, in the same idiom as the trajectory plots above (use `color_by = "group"` or `color_by = "trajectory"`).

The trajectory-level view needs every group's `detect_main_trajectories()` output, passed as `all_detected`. Here the stagnant trajectory `c1g3 tr3` (last seen in 2017) is mostly absorbed by `c1g3 tr1`, the group's main living lineage.

```{r eval = FALSE, warning = FALSE}
# detect_main_trajectories() for every group (used as `all_detected`)
grupos <- names(groups_cumulative_trajectories$groups_attributes)
groups_detected <- lapply(grupos, function(g) {
  birddog::detect_main_trajectories(groups_cumulative_trajectories, group = g)
})
names(groups_detected) <- grupos

# where do the papers of the stagnant trajectory c1g3 tr3 go?
dest <- birddog::sniff_trajectory_destination(
  detected       = groups_detected[["c1g3"]],
  traj_id        = "tr3",
  docs_per_group = groups_cumulative_trajectories$docs_per_group,
  all_detected   = groups_detected,
  group          = "c1g3"
)

# destination by absorbing trajectory (years on each path)
dest$destination_traj

birddog::plot_trajectory_destination(dest, color_by = "trajectory", min_prop = 0.03)
```

![](trajectory-destination-c1g3.png){width=99%}

### Which trajectories formed a living one

The mirror of the previous analysis: instead of following a stagnant trajectory forward, stand at a living trajectory and ask **which trajectories fed into it**. `sniff_trajectory_formation()` (and the batch `sniff_trajectory_formations()`) returns, for a target trajectory, every other trajectory whose terminal cohort was absorbed into it -- the exact inverse of `sniff_trajectory_destination()`. `plot_trajectory_formation_2d()` draws the target as a cumulative "river" with each feeder merging at its handoff year; `plot_trajectory_formation_3d()` is the interactive plotly version (X = year, Y = feeder lane, Z = documents).

```{r eval = FALSE, warning = FALSE}
# which trajectories formed the main living trajectory c1g3 tr1?
form <- birddog::sniff_trajectory_formation(
  target         = "c1g3::tr1",
  all_detected   = groups_detected,
  docs_per_group = groups_cumulative_trajectories$docs_per_group
)

form$feeders                       # feeders with document counts and handoff years

birddog::plot_trajectory_formation_2d(form, max_feeders = 8)
birddog::plot_trajectory_formation_3d(form, max_feeders = 8)
```

![](trajectory-formation-c1g3.png){width=99%}
![](trajectory-formation-3d-c1g3.png)

### Self-sufficiency

How much of each living trajectory grew on its own versus being a confluence of others? `sniff_trajectory_self_sufficiency()` reports `1 - imported / size`: values near 1 mean the lineage grew endogenously; lower values mean it absorbed many documents from other communities.

```{r eval = FALSE}
ss <- birddog::sniff_trajectory_self_sufficiency(
  all_detected   = groups_detected,
  docs_per_group = groups_cumulative_trajectories$docs_per_group,
  min_size       = 50
)
...
```{r include = FALSE, }
self_sufficiency <- readRDS(file.path(local_data, "self_sufficiency.rds"))
```
