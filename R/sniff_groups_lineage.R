...
#'
#' Prepares lineage data (`docs_per_group`, `groups_similarity`, `groups_attributes`);
#' detects no trajectories -- see [sniff_trajectory_braid()].
#'
...
#' @family groups (stock)
#' @export
sniff_groups_lineage <- function(groups_cumulative, min_group_size = 10, top_n_keywords = 3) {
  # Input validation
  ...
  tryCatch({
    ...
    stop("Error in sniff_groups_lineage: ", e$message)
  })
}
