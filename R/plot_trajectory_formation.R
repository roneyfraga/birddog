...
#'   range only extends the axis (it never drops data).
...
#'   by feeder `source_key`), to match colours across plots. `NULL` (default) uses
#'   the package's Set2 (<= 8 feeders) / hue scheme.
#'
...
#' }
#'
#' @seealso [sniff_trajectory_formation()], [plot_trajectory_lines_2d()],
...
#' @importFrom ggplot2 annotate geom_blank scale_fill_manual scale_colour_manual
...
#' @export
plot_trajectory_formation <- function(formation,
  ...
  all_feeders <- formation$feeders
  feeders <- all_feeders[all_feeders$kept %in% TRUE, , drop = FALSE]
  if (nrow(feeders) == 0) {
    if (nrow(all_feeders) == 0) {
      stop("'", formation$target %||% formation$target_info$traj_id, "' has no feeders ",
           "(no trajectory was absorbed into it), so there is no formation to plot",
           call. = FALSE)
    }
    stop("no feeders pass the min_papers / min_prop thresholds; lower them in ",
         "sniff_trajectory_formation() (not in this plot) to keep smaller feeders",
         call. = FALSE)
  }
  ...
  # name with the target's final-year size on a line below (the documents that
  # remain in the final community), matching plot_trajectory_lines_3d(); falls back
  # to the trajectory's total size for legacy objects without final_size.
  tsize <- if (!is.null(ti$final_size)) ti$final_size else ti$size
  target_text <- if (!is.null(tsize)) {
    paste0(target_label, "\n(", tsize, ")")
  } else {
    target_label
  }
  ...
  # feeder label: the documents that end in the target (n_dest, flow objects),
  # else the legacy inflow/cohort. Matches plot_trajectory_lines_3d / confluence.
  feeders$lab <- if ("n_dest" %in% names(feeders)) {
    paste0(feeders$source_key, "\n(", feeders$n_dest, ")")
  } else {
    paste0(feeders$source_key, "\n(", feeders$n, "/", feeders$cohort_size, ")")
  }
  # feeder colours: the package's Set2 (<= 8) / hue scheme keyed by source_key, or
  # a caller palette, so colours can be matched across plots (cf. lines_2d).
  keys <- unique(feeders$source_key)
  pal <- if (!is.null(palette)) palette else if (length(keys) <= 8) {
  ...
  # time axis: fit the drawn data, or the requested year_range window
  yr <- if (!is.null(year_range)) range(year_range) else
        c(min(feeders$start_year, ti$start), last_year)
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::scale_fill_manual(values = pal, guide = "none", na.value = "grey70") +
    ggplot2::scale_colour_manual(values = pal, guide = "none", na.value = "grey60") +
  ...
  if (!is.null(year_range)) {
    p <- p + ggplot2::geom_blank(
      data = data.frame(x = yr, y = c(spine_y, spine_y)),
