...
#' @title sniff_topological_positions
...
#'  \code{\link[stats]{sd}}
#' @rdname sniff_topological_positions
#' @export
...
sniff_topological_positions <- function(data, id, group, PY = PY, 
                                      TI = TI, CR = CR, 
                                      min.citations = 1) {
    ...
    M$nCITING <- 1:nrow(M)
    papers <- M$nCITING[as.numeric(M$TC) >= min.citations]
    ...
    M |> 
        ...
        dplyr::filter(.data$group == .data$cited_from) |> 
