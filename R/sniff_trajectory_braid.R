...
#'
#' @param x A [sniff_trajectory_dag()] object, a [sniff_groups_lineage()]
...
#'
#' @return A `birddog_flow` object: a classed list with `trajectories` (tibble:
...
#'
#' @seealso [sniff_trajectory_channel()], [sniff_trajectory_dag()],
...
sniff_trajectory_braid <- function(x, min_group_size = 10, jaccard_min = 0.05,
                                  k_out = 2) {
...
#' Print a flow-trajectory object
#'
#' @param x A `birddog_flow` object.
#' @param ... Unused.
#' @return `x`, invisibly.
#' @keywords internal
#' @export
print.birddog_flow <- function(x, ...) {
  tt <- x$trajectories
  cat("<birddog_flow> ", sum(tt$type == "central"), " central + ",
      sum(tt$type == "absorbed"), " absorbed trajectories, last year ",
      x$last_year, "\n", sep = "")
  if (!is.null(attr(x, "pruned"))) {
    cat("(pruned: ", attr(x, "pruned"), ")\n", sep = "")
  }
  print(utils::head(tt[tt$type == "central",
                       c("traj_id", "start", "end", "size")], 5))
  invisible(x)
}
